"""Add is_favourite column to games_master
...
def upgrade():
    ...
    # Recreate view to include is_favourite
    ...
    op.execute("""
        ...
        SELECT 
            ...
            g.total_rating, g.status, g.is_favourite,
...
def downgrade():
    ...
    op.drop_column('games_master', 'is_favourite')

    # Recreate view without is_favourite (matches c3d4e5f6a7b8 state)
    op.execute("""
        CREATE VIEW vw_owned_games_unified AS
        SELECT 
            g.game_id, g.title, g.cover_url, g.genres, g.first_release_date,
            ...
            og.playtime_minutes, og.last_played,
            CASE WHEN gfn.game_id IS NOT NULL THEN true ELSE false END as is_on_gfn,
            COALESCE(CASE WHEN gfn.gfn_game_id IS NOT NULL THEN 'https://play.geforcenow.com/games?game-id=' || gfn.gfn_game_id ELSE gfn.gfn_url END, NULL) as gfn_deeplink_url
        ...
        UNION ALL
        SELECT g.game_id, g.title, g.cover_url, g.genres, g.first_release_date, g.total_rating, g.status, p.name, a.username, og.ea_game_id::TEXT, NULL, NULL, CASE WHEN gfn.game_id IS NOT NULL THEN true ELSE false END, COALESCE(CASE WHEN gfn.gfn_game_id IS NOT NULL THEN 'https://play.geforcenow.com/games?game-id=' || gfn.gfn_game_id ELSE gfn.gfn_url END, NULL)
        FROM owned_games_ea og JOIN accounts a ON og.account_id = a.account_id JOIN platforms p ON a.platform_id = p.platform_id LEFT JOIN games_master g ON og.game_id = g.game_id LEFT JOIN gfn_games gfn ON g.game_id = gfn.game_id
        UNION ALL
        SELECT g.game_id, g.title, g.cover_url, g.genres, g.first_release_date, g.total_rating, g.status, p.name, a.username, og.battlenet_game_id::TEXT, NULL, NULL, CASE WHEN gfn.game_id IS NOT NULL THEN true ELSE false END, COALESCE(CASE WHEN gfn.gfn_game_id IS NOT NULL THEN 'https://play.geforcenow.com/games?game-id=' || gfn.gfn_game_id ELSE gfn.gfn_url END, NULL)
        FROM owned_games_battlenet og JOIN accounts a ON og.account_id = a.account_id JOIN platforms p ON a.platform_id = p.platform_id LEFT JOIN games_master g ON og.game_id = g.game_id LEFT JOIN gfn_games gfn ON g.game_id = gfn.game_id
        UNION ALL
        SELECT g.game_id, g.title, g.cover_url, g.genres, g.first_release_date, g.total_rating, g.status, p.name, a.username, og.ubisoft_game_id::TEXT, NULL, NULL, CASE WHEN gfn.game_id IS NOT NULL THEN true ELSE false END, COALESCE(CASE WHEN gfn.gfn_game_id IS NOT NULL THEN 'https://play.geforcenow.com/games?game-id=' || gfn.gfn_game_id ELSE gfn.gfn_url END, NULL)
        FROM owned_games_ubisoft og JOIN accounts a ON og.account_id = a.account_id JOIN platforms p ON a.platform_id = p.platform_id LEFT JOIN games_master g ON og.game_id = g.game_id LEFT JOIN gfn_games gfn ON g.game_id = gfn.game_id;
    """)
